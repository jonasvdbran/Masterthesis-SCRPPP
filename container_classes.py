# -*- coding: utf-8 -*-
"""Copy of Container_classes.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/13zQ6K-EXZAgD-Gx3QmAmZxuFEZ-xlbzE
"""
...
"""# Correct and acceptable stacks

"""

def IsCorrect(stack):
  # Checks if a stack is correct.
  return all(stack[j] > stack[j + 1] for j in range(len(stack) - 1))

def CorrectStacks(module):
  stacks=module.stacks
  n_cells=module.size()[1]

  # Returns a list with entries (stack_index,stack) if stack is correct.
  correct_stacks = [((i//n_cells,i%n_cells), stack) for i, stack in enumerate(stacks) if IsCorrect(stack)]
  return correct_stacks

def IsAcceptable(stack):
  # Checks if a stack is acceptable.
  return all(stack[j] >= stack[j + 1] for j in range(len(stack) - 1))

def AcceptableStacks(module):
    stacks = module.stacks
    acceptable_stacks = [(i, stack) for i, stack in enumerate(stacks) if IsAcceptable(stack)]
    return acceptable_stacks

"""Next, we want to check if there is a correct stack or acceptable stack for a container."""

def has_correct_stack(module,value,index:tuple):
  correct_stacks= CorrectStacks(module)
  for i,stack in correct_stacks:
    if stack.is_empty():
      return True
    elif min(stack).value>value and not(stack.is_full()) and i!=index:
      return True
  return False

def has_acceptable_stack(module,value,index:tuple):
  # Returns a list with the number of containers with same timeslot if there exist an acceptable stack (but is not correct)
  acceptable_stacks= AcceptableStacks(module)
  n_same_timeslots=[]
  for i,stack in acceptable_stacks:
    if stack.is_empty():
      return True
    elif min(stack).value>=value and not(stack.is_full()) and i!=index:
      n_same_timeslots.append(len([container.value for container in stack if container.value==value]))
  if any(n_same_timeslots):
    return n_same_timeslots
  else:
    return False

"""# Blocking Costs

Next, we implement the blocking costs, as defined in Lemma 8.
"""

def blocking_costs(module):
  b={}
  for i,bay in enumerate(module):
    for j,stack in enumerate(bay):
      for k,container in enumerate(stack):
        # Container has smaller time slot than one of the containers underneath
        if any(stack[k].value > below for below in stack[:k]):
          b[i,j,k]=1
        else:
          # Count the number of containers with the same time slot underneath.
...
"""# Look-ahead shifting costs.

Next, we implement the look-ahead shifting costs algorithm,algorithm 2. It estimates extra moves due to bad stacks. First we define a function that retrieves containers in the module.
"""

def retrieve_containers(module,retrieving_containers):
  stack_height=module.max_height
  retrieving_positions=[]
  # Retrieves the containers in retrieving_containers, together with the ones above them.
  # This removes them from the module.
  for c in retrieving_containers:
    retrieving_positions+= [(c[0],c[1],i) for i in range(c[2],len(module[c[0]][c[1]]))]
  new_module=[]
  for i,bay in enumerate(module):
    new_bay=[]
    for j,stack in enumerate(bay):
      new_stack=[]
      for k,container in enumerate(stack):
        if (i,j,k) not in retrieving_positions:
          new_stack.append(container)
      new_bay.append(Stack(new_stack,stack_height))
    new_module.append(Bay(new_bay,stack_height))
  return Module(new_module,stack_height)

"""The next code implements algorithm 2, together with the expected shifting costs defined in equation (3.4). That is
SM(c)=b(c)+∑_i d_i(c), where d_i is the extra shiftings due to bad stacks, in level i of the algorithm.
"""

def CalculateShiftingCosts(module,k_max=2):
  d = {}
    ...
    for x in range(len(module)):
      for y in range(len(module[x])):
        for z in range(len(module[x][y])):
          d[k,(x,y,z)]=0
  SC=blocking_costs(module)

  # Algorithm 2
  def EstimateBadShiftings(module,k=1,min_value=1):
    stacks=module.stacks
    # Up to depth k_max, or if any stack is empty stop because we can always put containers in this stack.
    if k>k_max or k<0 or any([stack.is_empty() for stack in stacks]):
        return
    T_min=module.where_value(min_value)
    n=len(T_min)
    if n==0:
      return

    for c in T_min:
      # Containers above are blocking c from being retrieved
      blocking_containers=module[c[0]][c[1]][c[2]+1:]
      for i,c_b in enumerate(blocking_containers):
        position=(c[0], c[1], c[2] + 1 + i)
        stack_index=(c[0],c[1])

        # If the blocking container has correct stack we don't add a cost
        if has_correct_stack(module,c_b,stack_index):
          continue
        # If it has an acceptable stack, add the blocking probability as cost, weighted by the probability of the scenario.
        elif has_acceptable_stack(module,c_b,stack_index):
          n_same_timeslots=has_acceptable_stack(module,c_b,stack_index)
          m=min(n_same_timeslots)+1
          d[k,position]+=1/math.prod(n + i for i in range(k))*(1-1/m)
        else:
          d[k,position]+=1/math.prod(n + i for i in range(k))
      # Go to the next depth/level
      if k<k_max:
        new_module=retrieve_containers(module,[c])
        EstimateBadShiftings(new_module,k+1,min_value)
    return

  max_container=module.max_container()
  for i in range(1,max_container.value+1):
    stacks=module.stacks
    if any([stack.is_empty() for stack in stacks]):
      break
    # Estimate the bad shiftings for the containers with time slot i
    EstimateBadShiftings(module,min_value=i)
    # Retrieve the containers with time slot i, and the ones above and go to the next time slot
    module=retrieve_containers(module,module.where_value(i))
